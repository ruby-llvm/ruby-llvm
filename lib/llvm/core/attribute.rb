...
module LLVM
  class Attribute
    ...
    def self.create_enum(kind, value = 0, context = Context.global)
      attr_id = attribute_id(kind)
      ptr = C.create_enum_attribute(context, attr_id, value)
      from_ptr(ptr)
    end

    def self.create_string(key, value, context = Context.global)
      key = key.to_s
      value = value.to_s
    ...
    def kind
      return C.get_enum_attribute_kind(self) if enum?
    ...
    def self.attribute_name(attr_name)
    ...
    def self.attribute_id(attr_name)
